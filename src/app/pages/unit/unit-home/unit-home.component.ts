...
export class UnitHomeComponent implements OnInit {
  ...
  readonly inputSearchFilterList: KeyOfData[] = ['idUnit', 'name', 'comment'];
  ...
  onRedirectToEditPage = (data: IUnit): void => {
    this.router.navigate([`/${this.currentView}/edit/${data.idUnit}`]);
  };

  onCloneRegister = (data: IUnit): void => {
    this.router.navigate([`/${this.currentView}/new/${data.idUnit}`]);
  };
  ...
  onShowDataList = async (): Promise<void> => {
    try {
      ...
      if (response.data) {
        const dataList = response.data as IUnit[];
  ...
  async onDelete(selectedData: IUnit): Promise<void> {
    await this.onDeleteRegister(selectedData.idUnit as number);
  }
  ...
  onShowModalToDeleteThroughTableBtn(data?: IUnit): void {
